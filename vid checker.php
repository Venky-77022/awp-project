...
<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $query = "SELECT * FROM registration WHERE username='$username' && mobilenumber='$mobilenumber'";
        ...
        if (mysqli_num_rows($result) == 1) {
			...
			if ($message->sid) {
				...
				echo "<script> window.location.assign('verifyotp1.php');</script>";
			}
            exit();
        } 
        else
